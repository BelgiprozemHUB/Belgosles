...
input_selection = a.GetParameterAsText(1)
is_update_geometry = a.GetParameter(2)
...
def main(input_folder=input_folder):
    ...
    a.Delete_management(path.join(a.env.scratchGDB, "Kvartal_SELECT"))
    ...
    try:
        a.MakeFeatureLayer_management(
            in_features=path.join(taxationDB, "FORESTS", "Kvartal"), 
            out_layer="Kvartal_Layer", 
            where_clause=input_selection)
        a.CopyFeatures_management(
            in_features="Kvartal_Layer", 
            out_feature_class=path.join(a.env.scratchGDB, "Kvartal_SELECT"))
        ...
        a.Delete_management("Kvartal_Layer")
        a.Buffer_analysis(
            in_features=path.join(a.env.scratchGDB, "Kvartal_line"),
        ...
        verification_GIDRO_name = "Verification_GIDRO_border_SELECT"
        if a.Exists(path.join(taxationDB, "CHANGES", verification_GIDRO_name)):
            suff = 1
            while True:
                if a.Exists(path.join(taxationDB, "CHANGES", verification_GIDRO_name + "_" + unicode(suff))):
                    suff += 1
                else:
                    verification_GIDRO_name += "_" + unicode(suff)
                    break
        a.MultipartToSinglepart_management(
            in_features=path.join(a.env.scratchGDB, "Verification_GIDRO_temp"), 
            out_feature_class=path.join(taxationDB, "CHANGES", verification_GIDRO_name))
        a.Delete_management(path.join(a.env.scratchGDB, "Verification_GIDRO_temp"))

        a.MakeFeatureLayer_management (
            in_features=path.join(taxationDB, "CHANGES", verification_GIDRO_name), 
            out_layer="GIDRO_border_Layer")
        a.SelectLayerByAttribute_management(
            in_layer_or_view="GIDRO_border_Layer", 
            selection_type="NEW_SELECTION", 
        ...
        a.CalculateField_management(
            in_table="GIDRO_border_Layer", 
            field="SHIR", expression="[SHIR]*2", 
            expression_type="VB", code_block="")
        a.Buffer_analysis(
            in_features="GIDRO_border_Layer",
        ...
        if int(a.GetCount_management(path.join(taxationDB, "CHANGES", verification_GIDRO_name))
            .getOutput(0)) == 0:
            a.Delete_management(path.join(taxationDB, "CHANGES", verification_GIDRO_name))
        else:
            a.AddMessage(u'     Создан класс для проверки гидрографии на границах кварталов: {}'
                .format(path.join(taxationDB, "CHANGES", verification_GIDRO_name)))

        union_fcs = path.join(a.env.scratchGDB, "Vydel_buffer_1") + u" #;" + \
                    path.join(a.env.scratchGDB, "Kvartal_SELECT") + u" #"
        a.Union_analysis(
            in_features=union_fcs, 
            out_feature_class=path.join(a.env.scratchGDB, "Vydel_buffer_1_Union"), 
            join_attributes="ALL", cluster_tolerance="", gaps="GAPS")
        a.MakeFeatureLayer_management (
            in_features=path.join(a.env.scratchGDB, "Vydel_buffer_1_Union"), 
            out_layer="Union_Layer", where_clause="NUM_KV = NUM_KV_1")
        a.Dissolve_management(
            in_features="Union_Layer", 
    ...
    try:
        ...
        a.Union_analysis(
            ...
            join_attributes="ALL", cluster_tolerance="", gaps="GAPS")
        a.MakeFeatureLayer_management (
            in_features=path.join(a.env.scratchGDB, "Vydel_buffer_2_Union"), 
            out_layer="Union_Layer", where_clause="NUM_KV = NUM_KV_1")
        a.Dissolve_management(
            in_features="Union_Layer", 
    ...
    try:
        ...
        a.Union_analysis(
            ...
            join_attributes="ALL", cluster_tolerance="", gaps="GAPS")
        a.MakeFeatureLayer_management (
            in_features=path.join(a.env.scratchGDB, "Vydel_buffer_3_Union"), 
            out_layer="Union_Layer", where_clause="NUM_KV = NUM_KV_1")
        a.Dissolve_management(
            in_features="Union_Layer", 
    ...
    try:
        ...
        a.Union_analysis(
            ...
            join_attributes="ALL", cluster_tolerance="", gaps="GAPS")
        a.MakeFeatureLayer_management(
            in_features=path.join(a.env.scratchGDB, "Vydel_buffer_4_Union"), 
            out_layer="Union_Layer", where_clause="NUM_KV = NUM_KV_1")
        a.Dissolve_management(
            in_features="Union_Layer", 
    ...
    try:
        ...
        a.Union_analysis(
            ...
            join_attributes="ALL", cluster_tolerance="", gaps="GAPS")
        a.MakeFeatureLayer_management(
            in_features=path.join(a.env.scratchGDB, "Vydel_buffer_5_Union"), 
            out_layer="Union_Layer", where_clause="NUM_KV = NUM_KV_1")
        a.Dissolve_management(
            in_features="Union_Layer", 
    ...
    try:
        ...
        if int(a.GetCount_management("Vydel_L_Layer").getOutput(0)) > 0:
            ...
            a.Union_analysis(
                ...
                join_attributes="ALL", cluster_tolerance="", gaps="GAPS")
            a.MakeFeatureLayer_management(
                in_features=path.join(a.env.scratchGDB, "Vydel_buffer_6_Union"), 
                out_layer="Union_Layer", where_clause="NUM_KV = NUM_KV_1")
            a.Dissolve_management(
                in_features="Union_Layer", 
    ...
    try:
        ...
        if a.Exists(path.join(a.env.scratchGDB, "Vydel_buffer_6_other")):
            a.Update_analysis(
                in_features=path.join(a.env.scratchGDB, "Vydel_buffer_3_proseki"),
            ...
            a.Update_analysis(
                in_features=path.join(a.env.scratchGDB, "Vydel_buffer_update_1"),
            ...
            a.Update_analysis(
                ...
                out_feature_class=path.join(a.env.scratchGDB, "Vydel_buffer_update_2"))
        a.Update_analysis(
            in_features=path.join(a.env.scratchGDB, "Vydel_buffer_update_2"),
        ...
        a.Delete_management(path.join(a.env.scratchGDB, "Vydel_buffer_update_2"))
        a.MakeFeatureLayer_management(
            in_features=path.join(taxationDB, "FORESTS", "Vydel"), 
            out_layer="Vydel_update_Layer", 
            where_clause=input_selection)
        a.Update_analysis(
            in_features="Vydel_update_Layer",
            update_features=path.join(a.env.scratchGDB, "Vydel_buffer_update_1"), 
            out_feature_class=path.join(a.env.scratchGDB, "Vydel_buffer_update_2"))
        a.Delete_management("Vydel_update_Layer")
        a.Delete_management(path.join(a.env.scratchGDB, "Vydel_buffer_update_1"))
        a.Dissolve_management(
            in_features=path.join(a.env.scratchGDB, "Vydel_buffer_update_2"),
            out_feature_class=path.join(a.env.scratchGDB, "Vydel_buffer_update_1"),
            dissolve_field=["FORESTCODE", "LESHOZKOD", "LESNICHKOD", 
                            ...
                            "AREA", "DELTA", "AREADOC"],
            statistics_fields="", multi_part="MULTI_PART")
        vydel_select_name = "Vydel_S_SELECT"
        if a.Exists(path.join(taxationDB, "FORESTS", vydel_select_name)):
            suff = 1
            while True:
                if a.Exists(path.join(taxationDB, "FORESTS", vydel_select_name + "_" + unicode(suff))):
                    suff += 1
                else:
                    vydel_select_name += "_" + unicode(suff)
                    break
        a.CreateFeatureclass_management(
            out_path=path.join(taxationDB, "FORESTS"), 
            out_name=vydel_select_name, 
            geometry_type="POLYGON", 
            template=path.join(taxationDB, "FORESTS", "Vydel_S"))
        a.Append_management(
            inputs=path.join(a.env.scratchGDB, "Vydel_buffer_update_1"), 
            target=path.join(taxationDB, "FORESTS", vydel_select_name), 
            schema_type="TEST")
    ...
    if is_update_geometry:
        a.AddMessage(u"\nIII. Обновление геометрии (дополнительная опция).")
        try:
            a.MakeFeatureLayer_management(
                in_features=path.join(taxationDB, "FORESTS", "Vydel_S"), 
                out_layer="Vydel_S_Layer")
            a.SelectLayerByAttribute_management(
                in_layer_or_view="Vydel_S_Layer", 
                selection_type="NEW_SELECTION", 
                where_clause=input_selection)
            a.DeleteFeatures_management(in_features="Vydel_S_Layer")
            a.Append_management(
                inputs=path.join(taxationDB, "FORESTS", vydel_select_name), 
                target="Vydel_S_Layer", 
                schema_type="TEST")
            a.Delete_management(path.join(taxationDB, "FORESTS", vydel_select_name))
            a.AddMessage(u"  Геометрия успешно обновлена.")
        except:
            a.AddWarning(u"  Не удалось обновить геометрию!")
            exit()
    else:
        a.AddMessage(u'  Создан класс полигональных выделов для выбранных кварталов: {}'
            .format(path.join(taxationDB, "FORESTS", vydel_select_name)))
